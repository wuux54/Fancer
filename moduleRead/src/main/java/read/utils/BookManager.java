...
public class BookManager {
    ...
    public boolean openChapter(String bookId, String chapterName, long position) {
        ...
        File file = new File(Constant.BOOK_CACHE_PATH + bookId
                + File.separator + chapterName + BookFileUtils.SUFFIX_NB);
        if (!file.exists()) {
            return false;
        }
        this.bookId = bookId;
    ...
    private void createCache() {
        ...
        if (!cacheMap.containsKey(chapterName)) {
            ...
            char[] array = BookFileUtils.getFileContent(file).toCharArray();
    ...
    public char[] getContent() {
        ...
        if (block == null) {
            File file = getBookFile(bookId, chapterName);
            block = BookFileUtils.getFileContent(file).toCharArray();
    ...
    public File getBookFile(String folderName, String fileName) {
        return BookFileUtils.getFile(Constant.BOOK_CACHE_PATH + folderName
                + File.separator + fileName + BookFileUtils.SUFFIX_NB);
    }

    public long getBookSize(String folderName) {
        return BookFileUtils.getDirSize(BookFileUtils
     ...
     * @return
     */
    public boolean hasBookFile(String folderName, String fileName) {
