...
import read.Constant;
import read.bean.BookBean;
import read.bean.TxtChapterBean;
import read.bean.TxtDrawBean;
import read.callback.ItfBookEventCallback;
...
public abstract class BasePageLoader {
    ...
    // 当前显示的页
    private TxtDrawBean mCurPage;
    // 上一章的页面列表缓存
    private List<TxtDrawBean> mPrePageList;
    // 当前章节的页面列表
    private List<TxtDrawBean> mCurPageList;
    // 下一章的页面列表缓存
    private List<TxtDrawBean> mNextPageList;
    ...
    private void initData() {
        ...
        // 初始化参数
        mMarginWidth = DensityUtils.dp2px(mContext, DEFAULT_MARGIN_WIDTH);
    ...
    private void setUpTextParams(int textSize) {
        // 文字大小
        mTextSize = textSize;
        mTitleSize = mTextSize + DensityUtils.sp2px(mContext, EXTRA_TITLE_SIZE);
        // 行间距(大小为字体的一半)
    ...
    private void initPaint() {
        ...
        mTipPaint.setTextSize(DensityUtils.sp2px(mContext, DEFAULT_TIP_SIZE)); // Tip默认的字体大小
...
//        }
     ...
     * @return
     */
    private List<TxtDrawBean> loadPageList(int chapterPos) throws Exception {
        // 获取章节
        TxtChapterBean chapter = mChapterList.get(chapterPos);
        // 判断章节是否存在
     ...
     * @return
     */
    protected abstract BufferedReader getChapterReader(TxtChapterBean chapter) throws Exception;
     ...
     * @return
     */
    protected abstract boolean hasChapterData(TxtChapterBean chapter);
    ...
    private void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        int tipMarginHeight = DensityUtils.dp2px(mContext, 3);
        if (!isUpdate) {
            /****绘制背景****/
            ...
            //擦除区域
            mBgPaint.setColor(mBgColor);
            canvas.drawRect(mDisplayWidth / 2, mDisplayHeight - mMarginHeight + DensityUtils.dp2px(mContext, 2), mDisplayWidth, mDisplayHeight, mBgPaint);
        }
        ...
        int polarHeight = DensityUtils.dp2px(mContext, 6);
        int polarWidth = DensityUtils.dp2px(mContext, 2);
        int border = 1;
        int innerMargin = 1;
        ...
        Rect polar = new Rect(polarLeft, polarTop, visibleRight,
                polarTop + polarHeight - DensityUtils.dp2px(mContext, 2));
        ...
        int outFrameBottom = visibleBottom - DensityUtils.dp2px(mContext, 2);
        ...
        float x = outFrameLeft - mTipPaint.measureText(time) - DensityUtils.dp2px(mContext, 4);
    ...
    boolean prev() {
        ...
        if (mStatus == STATUS_FINISH) {
            // 先查看是否存在上一页
            TxtDrawBean prevPage = getPrevPage();
            if (prevPage != null) {
                mCancelPage = mCurPage;
                mCurPage = prevPage;
    ...
    boolean next() {
        ...
        if (mStatus == STATUS_FINISH) {
            // 先查看是否存在下一页
            TxtDrawBean nextPage = getNextPage();
            if (nextPage != null) {
                mCancelPage = mCurPage;
                mCurPage = nextPage;
                mPageView.drawNextPage();
        ...
        try {
            ...
            if (mCurPageList != null) {
                if (mCurPageList.isEmpty()) {
                    ...
                    // 添加一个空数据
                    TxtDrawBean page = new TxtDrawBean();
    ...
    private void preLoadNextChapter() {
        ...
        //调用异步进行预加载加载
        Single.create(new SingleOnSubscribe<List<TxtDrawBean>>() {
            @Override
            public void subscribe(SingleEmitter<List<TxtDrawBean>> e) throws Exception {
                e.onSuccess(loadPageList(nextChapter));
            }
        }).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new SingleObserver<List<TxtDrawBean>>() {
                    @Override
                    ...
                    @Override
                    public void onSuccess(List<TxtDrawBean> pages) {
                        mNextPageList = pages;
                    }
     ...
     * @return
     */
    private List<TxtDrawBean> loadPages(TxtChapterBean chapter, BufferedReader br) {
        //生成的页面
        List<TxtDrawBean> pages = new ArrayList<>();
        //使用流的方式加载
            ...
            while (showTitle || (paragraph = br.readLine()) != null) {
                ...
                while (paragraph.length() > 0) {
                    ...
                    if (rHeight <= 0) {
                        ...
                        page.title = StringUtils.convertCC(chapter.getTitle());
     ...
     * @return:获取初始显示的页面
     */
    private TxtDrawBean getCurPage(int pos) {
        if (mPageChangeListener != null) {
     ...
     * @return:获取上一个页面
     */
    private TxtDrawBean getPrevPage() {
     ...
     * @return:获取下一的页面
     */
    private TxtDrawBean getNextPage() {
        int pos = mCurPage.position + 1;
     ...
     * @return:获取上一个章节的最后一页
     */
    private TxtDrawBean getPrevLastPage() {
    ...
    public interface OnPageChangeListener {
         ...
         */
        void requestChapters(List<TxtChapterBean> requestChapters);
         ...
         */
        void onCategoryFinish(List<TxtChapterBean> chapters);
