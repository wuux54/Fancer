package com.read.widget.animation;
...
public class ScrollPageAnim extends PageAnimation {
    ...
    // 被废弃的图片列表
    private ArrayDeque<BitmapView1> mScrapViews;
    // 正在被利用的图片列表
    private ArrayList<BitmapView1> mActiveViews = new ArrayList<>(2);
    ...
    private void initWidget() {
        ...
        for (int i = 0; i < 2; ++i) {
            BitmapView1 view = new BitmapView1();
    ...
    // 底部填充
    private Iterator<BitmapView1> downIt;
...
/*          //擦除其Bitmap(重新创建会不会更好一点)
            ...
            if (!isRefresh) {
                ...
                if (!hasNext) {
                    ...
                    for (BitmapView1 activeView : mActiveViews) {
                        activeView.top = 0;
    ...
    private Iterator<BitmapView1> upIt;
    ...
    public void resetBitmap() {
        isRefresh = true;
        // 将所有的Active加入到Scrap中
        for (BitmapView1 view : mActiveViews) {
            mScrapViews.add(view);
        }
        // 清除所有的Active
        mActiveViews.clear();
...
/*        //设置背景透明
    ...
    private static class BitmapView1 {
        Bitmap bitmap;
        Rect srcRect;
        Rect destRect;
        int top;
